...
PROMPTS=(
    "convert a vintage look", #single prompt로 해도 되고,
    "convert a cool tone image" #여러 prompt list로 넣어도 됨. 
)

NEGATIVE_PROMPT="upscaling" #자율. 넣지 않을시 인자 뺄 것. 

INPUT_DIR="/workspace/test"
OUTPUT_DIR="method1_jiwoochoi"
GPU=1

mkdir -p "${OUTPUT_DIR}"

#prompt, negative prompt, image, gpu만 건들고 내부 인자&옵션은 건들지 말아주세요
